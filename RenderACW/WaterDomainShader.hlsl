	...
	HS_CONSTANT_DATA_OUTPUT input, float2 UV : SV_DomainLocation, const OutputPatch<HS_CONTROL_POINT_OUTPUT, NUM_CONTROL_POINTS> patch)
{
	...
	uvPos.x = 1.5 *cos(pi2 * UV.x)* cos(pi2 * UV.y);
	uvPos.y = 0.2 * cos(pi2 * UV.x)* sin(pi2 * UV.y);
	uvPos.z = 1.5 * sin(pi2 * UV.x);

	Output.Position = float4(0.3 * uvPos, 1);

	

	Output.Position = mul(Output.Position, model);
	
	Output.Position.z += sin(Output.Position.y * 2.8f + Time.x) * 3.5;
